  ...
  private computeNextDirectionBFS(
    ...
    while (queue.length > 0 && !found) {
      ...
      for (const d of dirs) {
        ...
        // Si c'est un portail, on remplace (nx,ny) par la case de sortie du portail
        // const tile = this.map.getTile({ x: nx, y: ny });
        // if (tile === TileType.Teleport) {
        //   // getTeleportDestination prend juste pos en grille, renvoie pixels
        //   const pixelDest = this.map.getTeleportDestination({ x: nx, y: ny });
        //   if (pixelDest) {
        //     // Convertir ces pixels en grille
        //     const destGridX = Math.floor(pixelDest.x / TILE_SIZE);
        //     const destGridY = Math.floor(pixelDest.y / TILE_SIZE);
        //     nx = destGridX;
        //     ny = destGridY;
        //   }
        // }
